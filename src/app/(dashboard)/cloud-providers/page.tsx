...
function AWSLogo() {
  return (
    <div className="w-12 h-10 rounded-xl bg-amber-500/20 border border-amber-500/30 flex items-center justify-center font-black text-amber-300 text-sm shadow-sm flex-shrink-0">
      AWS
    </div>
  )
}
function GCPLogo() {
  return (
    <div className="w-12 h-10 rounded-xl bg-blue-500/20 border border-blue-500/30 flex items-center justify-center font-black text-blue-300 text-sm shadow-sm flex-shrink-0">
      GCP
    </div>
  )
}
function AzureLogo() {
  return (
    <div className="w-12 h-10 rounded-xl bg-sky-500/20 border border-sky-500/30 flex items-center justify-center font-black text-sky-300 text-sm shadow-sm flex-shrink-0">
      Azure
    </div>
  )
}
...
export default function CloudProvidersPage() {
  ...
  return (
    <div className="space-y-6 w-full min-w-0">
      ...
      <PageHeader
        title="Multi-Cloud Storage Integrations"
        subtitle="Manage read-only telemetry telemetry connectors across Amazon S3, Google Cloud & Azure Blob."
        badge={`${connectedCount} of 3 Live`}
        actions={
          <button onClick={() => setConnectModal(true)} className="btn-primary flex-shrink-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full min-w-0">
        <div className="card p-5 card-glow-emerald flex items-center gap-3.5 min-w-0">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[11px] font-black uppercase tracking-wider text-slate-400 truncate">Connection Status</p>
            <p className="text-2xl font-black text-emerald-400 tracking-tight truncate">{connectedCount} of 3 Live</p>
            <p className="text-[11px] text-slate-400 mt-0.5 truncate">Automated 15-min sync schedule</p>
          </div>
        </div>
        ...
        <div className="card p-5 card-glow-blue flex items-center gap-3.5 min-w-0">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[11px] font-black uppercase tracking-wider text-slate-400 truncate">Total Monitored Footprint</p>
            <p className="text-2xl font-black text-blue-400 tracking-tight truncate">12.8 TB</p>
            <p className="text-[11px] text-slate-400 mt-0.5 truncate">27 total bucket containers</p>
          </div>
        </div>
        ...
        <div className="card p-5 card-glow-purple flex items-center gap-3.5 min-w-0">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[11px] font-black uppercase tracking-wider text-slate-400 truncate">Total Multi-Cloud Spend</p>
            <p className="text-2xl font-black text-purple-400 tracking-tight truncate">₹1,24,500</p>
            <p className="text-[11px] text-slate-400 mt-0.5 truncate">Consolidated monthly billing</p>
          </div>
        </div>
      </div>
        ...
        {providers.map(p => {
          ...
          return (
            <div
              key={p.id}
              className={clsx(
                'card p-6 transition-all w-full min-w-0',
                ...
                <div className="flex items-start gap-4 min-w-0 flex-1">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2.5 flex-wrap">
                      <h3 className="text-base font-black text-white tracking-tight truncate">{p.name}</h3>
                      <span className={clsx(
                        'inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-black border flex-shrink-0',
                        isConnected
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 w-full lg:w-auto flex-shrink-0">
                    {[
                      ...
                      <div key={m.label} className="bg-slate-900/80 p-2.5 rounded-xl border border-slate-700/80 text-center min-w-[105px]">
                        ...
                        <p className="text-xs font-black text-white mt-0.5 truncate">{m.value}</p>
                      </div>
                    ))}
                  </div>
                )}
